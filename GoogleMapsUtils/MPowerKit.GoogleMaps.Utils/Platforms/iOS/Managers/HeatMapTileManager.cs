...
//using HeatmapTileLayer = Google.Maps.Utils.GMUHeatmapTileLayer;
...
public static class HeatMapExtensions
{
    public static NGradient ToNative(this Gradient gradient)
    {
        return new NGradient(
            gradient.Colors.Select(c => c.ToPlatform()).ToArray(),
    ...
    {
        return new NWeightedLatLng(latLng.LatLng.ToCoord(), latLng.Intensity);
    }
}