...
public class PinManager : IMapFeatureManager<GoogleMap, GMap, GoogleMapHandler>
{
    ...
    public virtual void Connect(GoogleMap virtualView, GMap platformView, GoogleMapHandler handler)
    {
        ...
        NativeView!.SetInfoWindowAdapter(VirtualView!.InfoWindowTemplate is DataTemplate ? new InfoWindowAdapter(VirtualView!) : null);
    }
    ...
    {
        var pin = Pins.Single(go => (NativeObjectAttachedProperty.GetNativeObject(go) as NPin)!.Id == e.Marker.Id);
...
public class InfoWindowAdapter : Java.Lang.Object, GMap.IInfoWindowAdapter
{
    ...
    {
        var pin = Map.Pins.Single(go => (NativeObjectAttachedProperty.GetNativeObject(go) as NPin)!.Id == marker.Id);
